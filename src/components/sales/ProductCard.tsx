...
export function ProductCard({ product, onBuy, onClose }: ProductCardProps) {
  ...
  return (
    <motion.div
      ...
      className="bg-gradient-to-br from-gray-900/95 to-gray-800/95 backdrop-blur-xl rounded-2xl border border-white/10 shadow-2xl overflow-hidden w-80"
    >
      {/* Product Visual */}
      <div className={`relative h-40 bg-gradient-to-br ${visual.gradient}`}>
        ...
        {/* Badge */}
        {visual.badge && (
          <div className="absolute top-3 left-3 px-2 py-1 rounded-md bg-white/20 backdrop-blur-sm text-white text-xs font-bold">
            {visual.badge}
          </div>
        )}
        ...
        <button
          onClick={onClose}
          className="absolute top-2 right-2 p-1.5 rounded-full bg-black/50 hover:bg-black/70 text-white/70 hover:text-white transition-colors"
        >
        ...
        <div className="flex items-center justify-between">
          ...
          {product.price_id && (
            <motion.button
              ...
              className="px-4 py-2 rounded-xl bg-gradient-to-r from-cyan-500 to-emerald-500 text-white font-medium shadow-lg shadow-cyan-500/25 hover:shadow-cyan-500/40 transition-shadow"
            >
              Buy Now
            </motion.button>
          )}
